...
//int 배열의 정수값 중에서 최대값/최소값 구하는 메소드-참조값을 인자로 전달하는 연습
 public class ObjectArgMethod {
	 public static void main(String[] args) {
		...
		int max;     //max초기값:1)배열 인덱스 0번값  2)비교될 다른 값들보다 작은값
					//비교 범위  1)배열 인덱스 1~마지막인덱스 2)0~마지막인덱스
		max=arr[0];		//1번으로 구현
		for (int i = 0; i < arr.length; i++) {
			if(arr[i]>max) {	//최대값은 다른값들보다 큰값이 되어야 하므로, 그렇지 않을경
				max=arr[i];		//max의 ㅊ값을 그 값으로 변경 합니다
			}
	
		}
		System.out.println("최대값:"+max);
	
	 int min;			//min초기값:1)배열 인덱스 0번값  2)비교될 다른 값들보다 큰값
						//비교 범위  1)배열 인덱스 1~마지막인덱스 2)0~마지막인덱스
	 ...
	 System.out.println("메소드로 구한 합계:"+arrayAvgValue(arr));
	 
	 
	 double[] darr = new double [10];
	 for (int i = 0; i < darr.length; i++) {
		//darr[i]=r.nextDouble();    0~1사이 실수
	 ...
	 //최대값&최소값을 구하는 메소드 :메소드 오버로딩(over loading)-메소드 이름은 같으나 인자 형식이 다릅니다
	 //													-반환값은 같거나 다른타입 형식이 가능
	//			대표적인 메소드 -Arrays.toSttring() 메소드 입니다
	 
	 
	 
	 
	 }
//메소드 작성으로 구현:최대값/최솟값을 구하기 위해 다룰 데이터는 배열에 있습니다->배열의 변수는 참조 변수
//					->메소드 인자가 참조 타입이 됩니다->데이터 자체를 전달하지 ㅇ낳고 데이터가 저장된 주소를 전달.
